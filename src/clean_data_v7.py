...
def main():
    ...
    required_cols = {"city","cost"}
    if not required_cols.issubset(df_raw.columns):
        logging.info(f"Missing required column(s): {required_cols - df_raw.columns}")
        raise SystemExit(f"❌ Missing required column(s): {required_cols - df_raw.columns}")
    ...
    if not OUTPUT_PATH.parent.exists():             # Make sure parent folder exists
        logging.info(f"Creating output directory: {OUTPUT_PATH.parent}")
        OUTPUT_PATH.parent.mkdir(parents=True)
    # OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)     # alternative to 'if not' function → 'exist_ok=True': skip if exists
